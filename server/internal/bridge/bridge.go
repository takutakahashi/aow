...
func spawnAgentProc(cfg Config) (*agentProc, error) {
	...
	go func() {
		...
		if err := scanner.Err(); err != nil {
			log.Printf("[bridge] stdout scanner error (pid=%d): %v", cmd.Process.Pid, err)
		}
		log.Printf("[bridge] agent stdout closed (pid=%d)", cmd.Process.Pid)
	}()
		...
		for {
			...
			case line, ok := <-sub:
				...
				if err := conn.WriteMessage(websocket.TextMessage, line); err != nil {
					log.Printf("[bridge] WS write error: %v", err)
					return
				}
